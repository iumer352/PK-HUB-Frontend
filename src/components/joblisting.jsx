...
${jobPosting.keySkillsAndCompetencies}
            ...
            const parserResponse = await axios.post('http://10.183.199.14:8000/fastapi/parse-and-rank', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
            console.log('Parser API Request Details:', {
                url: 'http://10.183.199.14:8000/fastapi/parse-and-rank',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (jobId) {
                await axios.put(`http://10.183.199.14:5000/api/jobs/${jobId}`, jobPosting);
                setSuccessMessage('Job updated successfully');
                setTimeout(() => setSuccessMessage(''), 3000); // Clear message after 3 seconds
            } else {
                await axios.post('http://10.183.199.14:5000/api/jobs', jobPosting);
                navigate('/manage');
            }
        } catch (err) {
